gulf.controller("InactiveMechaicCtrl", function ($scope, $rootScope, $http, TransactionService, AppService, $state, HeaderService, $ionicPopup, $ionicPlatform, AddMechanicService, LoaderService, ConverBase64) {
    ...
    $(document).ready(function (e) {
        ...
        $('#soListInActive').select2({
            dropdownParent: $('#parentSoListInActive')
        });
        $('#thListInActive').select2({
            dropdownParent: $('#parentThListInActive')
        });
        $('#ahListInActive').select2({
            dropdownParent: $('#parentAhListInActive')
        });
    });
    ...
    $scope.search = function () {
        ...
        $('#inActiveMechnaic').DataTable({
            ...
            "ajax": {
                url: AppService.url + "api_r3/public/getSODetailedMechDetails?soId=" + $rootScope.userID + "&status=inactive&fromDate=" + $scope.fromdate + "&toDate=" + $scope.todate + "&areaName=" + $scope.data.selectedAreaInActiveMechanic + "&soFilter=" + selectedSOId + "&thFilter=" + selecteduserThId + "&ahFilter=" + selecteduserAHId + "&token=" + token,
                "type": "GET",
                dataType: "json",
    ...
    try {
        if (AppService.enableTracking) {
            _paq.push(['setDocumentTitle', "Mechanic List Page"]);
