gulf.controller("ActiveMechaicCtrl", function ($scope, $rootScope, $http, TransactionService, AppService, $state, HeaderService, $ionicPopup, $ionicPlatform, AddMechanicService, LoaderService, ConverBase64) {
    ...
    $scope.search = function () {
        ...
        $('#activeMechnaic').DataTable({
            ...
            "ajax": {
                url: AppService.url + "api_r3/public/getSODetailedMechDetails?soId=" + $rootScope.userID + "&status=Active&fromDate=" + $scope.fromdate + "&toDate=" + $scope.todate + "&areaName=" + $scope.data.selectedAreaInActiveMechanic + "&soFilter=" + selectedSOId + "&thFilter=" + selecteduserThId + "&ahFilter=" + selecteduserAHId + "&token=" + token,
                "type": "GET",
                dataType: "json",
