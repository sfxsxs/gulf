gulf.controller('verifyCodeResultCtrl', function ($scope, $state, $rootScope, $ionicPopup, $cordovaBarcodeScanner, $ionicPlatform, CodeVerificationservice, HeaderService, AppService) {
    ...
    var codeUsed = $rootScope.verifyResultIsUsed;
    $scope.newCashCoupon = $rootScope.newCashcouponValue;
    $scope.nonMlp;
    ...
    if (codeUsed == "0") {
        $scope.isCodeUsed = false;
    } else {
        $scope.isCodeUsed = true;
    }
    if ($scope.newCashCoupon == "0") {
        $scope.isnewCashCoupon = false;
    } else {
        $scope.isnewCashCoupon = true;
    }
    ...
    try {
        if (AppService.enableTracking) {
            _paq.push(['setDocumentTitle', "Verify Code Result Page"]);
