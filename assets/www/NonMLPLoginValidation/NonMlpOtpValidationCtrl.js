gulf.controller('NonMlpOtpValidationCtrl', function ($scope, LoginService, $state, $rootScope, ConverBase64, $ionicPopup, LoaderService, LoaderService, CodeCheckInService, $cordovaDevice, $ionicPlatform, AppService) {
    var mob;
    // mob = $rootScope.NonMlpRecievedMobile.toString();
    //         mob = ConverBase64.convertBase64(mob);
    mob = $rootScope.encoded_usd_mobile;
            ...
            CodeCheckInService.checkOTP(otp, mob).then(function (result) {
                ...
                if (result != "") {
                    if (result.Status == true) {
                        $rootScope.NonMlpUSerID = result.data[0].userId;
    ...
    try {
        if (AppService.enableTracking) {
            _paq.push(['setDocumentTitle', "Non MLP OTP Validation Page"]);
